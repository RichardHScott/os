...
impl Page {
    pub fn containing_address(address: VirtualAddress) -> Page {
        assert!(address < 0x0000_8000_0000_0000 || address >= 0xffff_8000_0000_0000,
            "Invalid address 0x{:x} due to sign bits", address);
...
impl ActivePageTable {
    ...
    pub fn unmap<A>(&mut self, page: Page, allocator: &mut A) where A : FrameAllocator {
        ...
        //allocator.deallocate_frame(frame); //todo, make sure allocator is REFERENCE COUTNED as if we unmap a frame, the frame might still be an entry in another page table
    }
}

unsafe fn invalidate_TLB(page : Page) {
    asm!("invlpg (%rax)" :: "{rax}"(page.start_address()) :: "volatile" );
}
